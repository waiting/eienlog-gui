...
public:
    /** \brief 枚举事件函数 */
    using EvtFn = std::function< void ( int fd, uint32_t events, void * data ) >;
    ...
    size_t evtsCount() const { return _evtsCount; }
...
private:
    ...
    std::vector<epoll_event> _evts; // 最大事件返回
    size_t _evtsCount; // 返回事件数量
};
